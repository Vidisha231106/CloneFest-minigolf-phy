...
export class EnhancedScoringDisplay {
  ...
  bindHUDEvents() {
    ...
    // Add game control button listeners
    document.getElementById('resetBtn')?.addEventListener('click', () => {
      if (window.ballPhysics) {
        window.ballPhysics.resetToStart();
        this.updateStrokes(0);
        if (document.getElementById('holeComplete')) {
          document.getElementById('holeComplete').style.display = 'none';
        }
      }
    });
    ...
    document.getElementById('followBtn')?.addEventListener('click', () => {
      if (window.followBall !== undefined) {
        window.followBall = !window.followBall;
        document.getElementById('followBtn').innerText = `Follow: ${window.followBall ? 'On' : 'Off'}`;
      }
    });
  }
